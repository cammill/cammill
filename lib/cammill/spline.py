...
execfile("/tmp/spline.points")

num_points = len(spline_points) * 10
spline = CubicSpline(spline_points)
calc = spline.approximate(num_points)

pl = calc.next()
for i in range(0, num_points - 1):
    p = calc.next()
    print str(pl[0]) + " " + str(pl[1]) + " " + str(p[0]) + " " + str(p[1])
    pl = p
